...
public class S2 implements IstateActions {
    ...
    public void onStateEntry(LinearLayout layout, Intent I, Context c,
        ...
        if (toastOrNot == true) {
            Toast.makeText(context, "لقد بدأت اللعبة", Toast.LENGTH_SHORT)
                    .show();
            toastOrNot = false;
        ...
        String Path = Environment.getExternalStorageDirectory().toString()
                + "/xGame/Games/الهجاء/Sound/start.mp3";
        // score sound
    ...
    public void createUI(LinearLayout layout, final Intent I, final Context c) {
        ...
        String alphabet = "ضصثقفغعهخحجدشسيبلاتنمكطئءؤرﻻىةوزظ";
        btn = new TextView[33];
        ...
        btn[32] = new TextView(c);

        StateListDrawable first = new StateListDrawable();
        first.addState(new int[]{android.R.attr.state_pressed}, bg2);
        first.addState(new int[]{android.R.attr.state_enabled}, bg);
        btn[32].setBackground(first);
        btn[32].setText(alphabet.charAt(32) + "");
        btn[32].setTextSize(25);
        ...
        float h = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 55,
                res.getDisplayMetrics());
        btn[32].setWidth((int) w);
        btn[32].setHeight((int) h);
        btn[32].setGravity(Gravity.CENTER);
        btn[32].setTextColor(Color.WHITE);
        btn[32].setTextSize(15);
        Spec row4 = GridLayout.spec(6, 3);
        Spec colspan2 = GridLayout.spec(2, 1);
        letters.addView(btn[32], new GridLayout.LayoutParams(row4, colspan2));
        btn[32].setOnClickListener(handleOnClick(btn[32], 32, I));
        ...
        return new View.OnClickListener() {
            public void onClick(View v) {
                String Path = Environment.getExternalStorageDirectory()
                        .toString() + "/xGame/Games/الهجاء/Sound/Button.mp3";
                // score sound

...
public class xGameList extends ArrayAdapter<Game> {
	...
	public View getView(int position, View view, ViewGroup parent) {
		...
		isWriteExternalPermissionGranted = appSharedPrefs.getBoolean("per",
				false);
		if (position % 2 == 0)
		...
		if (isWriteExternalPermissionGranted == true) {
			if (installations
					.isOfflineGameExists(games.get(position).getName()) != null
					&& !games.get(position).getVersion().equals(offlineVersion)) {
				rowView.setClickable(false);
				statusIcon.setImageResource(R.drawable.update);
				String contentD = "This game needs to be updated, click to download the latest version";

				if (lang.equals("AR"))
					contentD = "يوجد تحديث جديد متاح لهذه اللعبة..من فضلك إضعط لتحميل التحديث";
				statusIcon.setContentDescription(contentD);
			} else if (installations.isOfflineGameExists(games.get(position)
					.getName()) != null) {
				rowView.setClickable(false);
				statusIcon.setImageResource(R.drawable.start);
				String contentD = "This game is already installed, click to start playing, or long click to delete";
				if (lang.equals("AR"))
					contentD = "تم تحميل هذه اللعبة من قبل..إضغط لتبدأ اللعب.أو إضغط طويلا لإلغائها";
				statusIcon.setContentDescription(contentD);
			}
				...
				String contentD = "This game is being installed, please wait";
				if (lang.equals("AR"))
					contentD = "يتم اﻻن تحميل اللعبة ..من فضلك إنتظر قليلا";
				statusIcon.setContentDescription(contentD);
			}
				...
				String contentD = "This game is not yet installed, click to install";
				if (lang.equals("AR"))
					contentD = "لم يتم تحميل هذه اللعبة من قبل..من فضلك إضغط لتحميلها ثم بدء اللعب";
				statusIcon.setContentDescription(contentD);
			}
		} else {
			rowView.setClickable(true);
			statusIcon.setImageResource(R.drawable.closerec);
			String contentD = "Cannot find the game status, permission is not granted, refresh the list by swiping it down to request the permission again";
			if (lang.equals("AR"))
				contentD = "ﻻ يمكن تحديد حالة اللعبة الآن, لم يتم إعطاء الإذن للتطبيق بذلك,إسحب على القائمة لأسفل بإصبعين لطلب الإذن مرة أخرى";
			statusIcon.setContentDescription(contentD);
		}

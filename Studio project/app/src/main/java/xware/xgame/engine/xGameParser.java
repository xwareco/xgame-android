...
import xware.engine_lib.interfaces.IStateListener;
import xware.engine_lib.sensors.Accelerometer;
import xware.engine_lib.sound.HeadPhone;
import xware.engine_lib.R;
import xware.engine_lib.xml.State;
import xware.engine_lib.xml.StateEvent;
import xware.engine_lib.xml.Transition;
import xware.engine_lib.xml.XmlParser;
...
public class xGameParser extends Activity implements IStateListener {
	...
	private void startTheGame() {
		...
		if (currentState == null) {
			String lang = appPrefs.getString("Lang","EN");
			if (lang.equals("AR")) {
				Toast.makeText(xGameParser.this,
						"حدث خطأ أثناء تحميل اللعبة, من فضلك حاول مرة أخرى",
	...
	public void onBackPressed() {
		...
		if (count == 1) {
			...
			String textToToast = "Press Back again to exit the game";
			String lang = appPrefs.getString("Lang","EN");
			if (lang.equals("AR"))
				textToToast = "إضغط زر العودة مرة أخرى لإنهاء اللعبة";
			Toast.makeText(xGameParser.this, textToToast, Toast.LENGTH_LONG)
					.show();
			ed.putInt("cnt", count);
			ed.commit();
		} else if (count == 2) {

...
String[] words = new String[]{"كم تبلغ من العمر","ما هو لونك المفضل ؟","قم باستذكار دروسك اولا",
			"اعمل بجد لكى تنجح","قم بالذهاب الى مدرستك مبكرا","قم بترتيب غرفتك يوميا","من فضلك اغلق الباب"
			,"متى سوف تنتهى من واجبك ؟","تحلى بالصفات الحميدة دائما","استثمر وقتك فى الاشياء المفيدة","صاحب الاخيار وابتعد عن الاشرار"
			,"عامل والديك معاملة حسنة","عليك بتعلم اللغة الانجليزية"
			};
    ...
    return new View.OnClickListener() {
		...
		//	toastchield.rightMargin = 8;
			Drawable er = Drawable.createFromPath(Environment
					...
					+ "/xGame/Games/"
					+ "تكوين الجمل" + "/Images/error.png");
			errorImage.setImageDrawable(er);
			errorImage.setContentDescription("خطأ");
        	...
        	if(!(btn[index].getText().toString().equals("")))
        	{
        		...
        		{
        			if(myArabicCollator.compare(btn[index].getText().toString(),wordsArray[0])==0)
        			{
        		...
        		{
        			if(myArabicCollator.compare(btn[index].getText().toString(),wordsArray[1])==0)
        				 {
        				toast.show();
        		...
        		{
        			if(myArabicCollator.compare(btn[index].getText().toString(),wordsArray[2])==0)
        				 {
        				toast.show();
        		...
        		{
        			if(myArabicCollator.compare(btn[index].getText().toString(),wordsArray[3])==0)
        			{
        				toast.show();
...
class RemindTask extends TimerTask {
    ...
    public void run() {
    	...
    	String Path = Environment.getExternalStorageDirectory().toString() + "/xGame/Games/تكوين الجمل/Sound/timer.mp3";
		Hc.setLeftLevel(1);
   		Hc.setRightLevel(1);
		Hc.play(Path, 0);
      time++;

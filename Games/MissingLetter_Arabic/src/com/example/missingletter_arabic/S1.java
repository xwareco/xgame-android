...
public class S1  implements IstateActions {
	String[] words = new String[]{"مصر","السلام","الشكر","قرش","منتج","عالم","اختيار",
			"مدرسة","منزل","اهتمام","رسالة",
			"صورة","ارتفاع","ملاعب","عمل","معركة","جيوش","تخيل","احتفال","بحار","نهاية","بلدان","نقود","محبة",
			"طلب","مدفوعات","غابات","اشجار","ايمان","بيع",
			"ترتيب","اختيار","بشرية","اخلاق","الصدق","الشرطة","القومية","ايجابى","صديق","اقمار","ضرورى",
			"تغير","امان","زيادة","اسواق","راى","خطا","اهداف",
			"مخرج","نصيحة","فريق","ملاهى","عدالة","تعليمات","مناقشات","مقبول","تعليم",
			"انتاج","صناعة","عميق","اعلان","اصوات","اختيار","نباتات","حرية","مفترس","طالب","عروض",
			"ممارسة","تحقيق","تحسين",
			"استثمار","معرفة","فقدان"};
...
// edit test
		...
		Random r = new Random();
		char ch = (char)(r.nextInt(25) + 'ب');
		   firstLetter.setText(ch+"");
		   firstLetter.setTextSize(20);
		    ch = (char)(r.nextInt(25) + 'ب');
		...
		firstLetter.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				String Path = Environment.getExternalStorageDirectory().toString() + "/xGame/Games/الحرف المفقود/Sound/Button_best.mp3";
				//score sound
				...
				if (HP.detectHeadPhones() == true)
					HP.play(Path, 0);
				if(myArabicCollator.compare(firstLetter.getText().toString(),letter+"")==0)
				{
...
secondLetter.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (HP.detectHeadPhones() == true)
					HP.play(Path, 0);
				if(myArabicCollator.compare(secondLetter.getText().toString(),letter+"")==0)
				{
					 I.putExtra("timeInSecond",time);
...
thirdLetter.setOnClickListener(new View.OnClickListener() {
	...
	public void onClick(View v) {
		...
		if (HP.detectHeadPhones() == true)
			HP.play(Path, 0);
		if(myArabicCollator.compare(thirdLetter.getText().toString(),letter+"")==0)
		{
			 I.putExtra("timeInSecond",time);
...
forthLetter.setOnClickListener(new View.OnClickListener() {
	...
	public void onClick(View v) {
		...
		if (HP.detectHeadPhones() == true)
			HP.play(Path, 0);
		if(myArabicCollator.compare(forthLetter.getText().toString(),letter+"")==0)
		{
			 I.putExtra("timeInSecond",time);
...
class RemindTask extends TimerTask {
    ...
    public void run() {
    	...
    	String Path = Environment.getExternalStorageDirectory().toString() + "/xGame/Games/الحرف المفقود/Sound/timer.mp3";
		Hc.setLeftLevel(1);
   		Hc.setRightLevel(1);
		Hc.play(Path, 0);
      time++;
